...
def data_preprocessor(data, miss_rate, x_dim, y_dim):
    '''
    Preprocessing the data images by cropping the images and adding synthetic missing data
    
    Args:
    - data: the data array
    - miss_rate: the probability of missing data
    
    Returns:
    -data_x: original data
    -miss_data_x: data with added missing values
...
def show_results(data_x, miss_data_x, data_m, imputed_data, num_examples=1, cmap=None):
    for i in range(num_examples):
        ...
        # Only for missing values
        nominator = np.sum(((1-mask) * actual - (1-mask) * predicted)**2)
        ...
        plt.subplot(1,4,1)
        plt.title('Original')
        plt.imshow(actual.reshape(-1,int(actual.shape[0]**0.5)), cmap=cmap)
        plt.subplot(1,4,2)
        plt.title('Original With NaN')
        plt.imshow(miss_data.reshape(-1,int(actual.shape[0]**0.5)), cmap=cmap)
        plt.subplot(1,4,3)
        plt.title('NaN Mask')
        plt.imshow((1-mask).reshape(-1,int(actual.shape[0]**0.5)), cmap=cmap)
        plt.subplot(1,4,4)
        plt.title('Imputed')
        plt.imshow(predicted.reshape(-1,int(actual.shape[0]**0.5)), cmap=cmap);
        plt.show()