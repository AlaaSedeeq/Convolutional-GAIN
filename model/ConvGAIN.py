...
ConvGainUtils = SourceFileLoader("ConvGainUtils", "./utils/ConvGainUtils.py").load_module()
...
def Conv_GAIN(miss_data_x, conv_gain_parameters):
    ...
    # Normalization
    norm_data, norm_parameters = ConvGainUtils.normalization(miss_data_x)
    ...
    D_weights = {
        # Convolution Layers
        'c1': tf.Variable(ConvGainUtils.xavier_init((3,3,1,32)), name='W1'), 
        'c2': tf.Variable(ConvGainUtils.xavier_init((3,3,32,64)), name='W2'),

        # Dense Layers
        'd1': tf.Variable(ConvGainUtils.xavier_init((d_last_w, d_last_w//3)), name='W3'),
        'out': tf.Variable(ConvGainUtils.xavier_init((d_last_w//3, rows*cols)), name='W4'), 
    }
    D_biases = {
        # Convolution Layers
        'c1': tf.Variable(ConvGainUtils.xavier_init((32,)), name='B1'),
        'c2': tf.Variable(ConvGainUtils.xavier_init((64,)), name='B2'),

        # Dense Layers
        'd1': tf.Variable(ConvGainUtils.xavier_init((d_last_w//3,)), name='B3'),
        'out': tf.Variable(ConvGainUtils.xavier_init((rows*cols,)), name='B4'),
    }
    ...
    G_weights = {
        ...
        # Dense Layers
        'd1': tf.Variable(ConvGainUtils.xavier_init((g_last_w, g_last_w//3)), name='W3'),
        'out': tf.Variable(ConvGainUtils.xavier_init((g_last_w//3, rows*cols)), name='W4'),
    }
    G_biases = {
        # Convolution Layers
        'c1': tf.Variable(ConvGainUtils.xavier_init((32,)), name='B1'),
        ...
        # Dense Layers
        'd1': tf.Variable(ConvGainUtils.xavier_init((g_last_w//3,)), name='B3'),
        'out': tf.Variable(ConvGainUtils.xavier_init((rows*cols,)), name='B4'),
    }
...
#         conv2 = tf.nn.lrn(conv2, 4, bias=1.0, alpha=0.001/9.0, beta=0.75)
    ...
    for epoch in range(iterations):
        ...
        for step in prog_bar:  
            # Sample batch
            batch_idx = ConvGainUtils.sample_batch_index(no, batch_size)
            X_mb = norm_data_x[batch_idx, :] 
            X_mb = np.reshape(X_mb, (batch_size, rows, cols, 1))
            M_mb = data_m[batch_idx, :]  
            M_mb = np.reshape(M_mb, (batch_size, rows, cols, 1))
            # Sample random vectors  
            Z_mb = ConvGainUtils.uniform_sampler(0, 0.01, (batch_size, rows, cols, 1)) 
            Z_mb = np.reshape(Z_mb, (batch_size, rows, cols, 1))
            # Sample hint vectors
            H_mb_temp = ConvGainUtils.binary_sampler(hint_rate, (1, rows, cols, 1))
            H_mb = M_mb * H_mb_temp
    ...
    # Return imputed data
    # Z vector
    Z_mb = ConvGainUtils.uniform_sampler(0, 0.01, (no, rows, cols, 1)) 
